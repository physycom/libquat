...
int main(){
  ...
  for (int i = 0; i < 4; i++){
    std::cout << i << "  " << q1[i] << "  " << q2[i] << "  " << q3[i] << std::endl;
  }

  std::cout << std::endl << std::endl << "************* 2 - QUATERNIONI ROTANTI *************\n";
  double vers_x[] = { 1, 0, 0 },
    vers_y[] = { 0, 1, 0 },
    vers_z[] = { 0, 0, 1 };
  ...
  std::cout << std::endl << std::endl << "************* 3 - QUATERNIONI E CROSS PRODUCT *************\n";
  
  Quaternione q_xvy = .5*(q_x*q_y - q_y*q_x),
              q_yvz = .5*(q_y*q_z - q_z*q_y),
              q_zvx = .5*(q_z*q_x - q_x*q_z);

  std::cout << "x cross y" << endl; stampa_q(q_xvy); 
  std::cout << "y cross z" << endl; stampa_q(q_yvz); 
  std::cout << "z cross x" << endl; stampa_q(q_zvx);
